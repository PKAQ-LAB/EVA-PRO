...
export default ((window, undefined) => {
  ...
  function readyDom(iObj) {
    ...
    holder.style.width =
      matrix[1] * (radius * 2 + margin * 2) + margin * 2 + 'px';
    holder.style.height =
      matrix[0] * (radius * 2 + margin * 2) + margin * 2 + 'px';
  ...
  var startHandler = function (e, obj) {
    ...
    endHandler = (e, obj) => {
      ...
      iObj.holder.removeEventListener(
        'touchmove',
        window.__patternLockMoveHandler,
      );
      iObj.holder.removeEventListener(
        'mousemove',
        window.__patternLockMoveHandler,
  ...
  InternalMethods.prototype = {
    ...
    addLine: function (posObj) {
      ...
      if (patternAry.length > 1) {
        //to fix line
        var lA = getLengthAngle(this.lineX1, newX, this.lineY1, newY);
        this.line.style.width = lA.length + 10 + 'px';
        this.line.style.transform = 'rotate(' + lA.angle + 'deg)';

        if (!lineOnMove) this.line.style.display = 'block';
      }
      ...
      if (!lineOnMove) this.line.style.display = 'none';
    },
  ...
  function PatternLock(selector, option) {
    ...
    option = iObj.option = Object.assign(
      {},
  ...
  PatternLock.prototype = {
    ...
    reset: function () {
      ...
      iObj.holder
        .querySelectorAll('.patt-lines')
        .forEach((el) => iObj.wrapper.removeChild(el));
